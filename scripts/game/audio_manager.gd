...
## Global audio dispatcher. Load by short name; play SFX, voice, ambient, or music.
## Ambient/music are single-channel (new call replaces previous with crossfade).
...
const STREAMS := {
	...
	"animal_crossing":     "uid://cowls02kw013v",
...
# Names that should loop when used as ambient (we set loop_mode at load time
# so play_ambient doesn't need to know per-track details).
const LOOPABLE := [
...
var _ambient_secondary: AudioStreamPlayer  # used to crossfade
var _layer_player: AudioStreamPlayer  # stacked on top of the primary ambient
...
func play_ambient(track: String, fade_in: float = 1.0, volume_db: float = -6.0) -> void:
	...
	# Move current to secondary and fade it out while the new one fades in.
...
# -- Layered ambient (stacks on top of primary ambient) ---------------------
...
# -- Stop everything (used between scenes if you want a clean slate) ---------
